...
const checkInterval = setInterval(() => {
  if (tidalAuth.isInitialized.value) {
    tidalAuth.checkAuth();
  }
}, 30000);
onUnmounted(() => {
...
function handleLoginClick() {
  if (tidalAuth.isUserLoggedIn.value) {
    return;
  }
...
<template>
  <div class="flex items-center gap-2">
    <UButton
      v-if="!tidalAuth.isUserLoggedIn.value"
      icon="i-heroicons-user"
      variant="soft"
      size="sm"
      :loading="tidalAuth.isLoading.value"
